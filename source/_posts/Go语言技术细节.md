...
2. 常见用法
   ...
   - 使用for-range读取channel, 不需要判断chan是否关闭，如果关闭会自动退出for循环
   ...
   - 使用select读取多个channel, 如果ch为nil值会阻塞，case永远为阻塞，无论读写。普通情况下写nil chan会panic
...
1. defer 函数执行过程
   ...
   - deferproc
      1. deferproc是在函数执行流中调用，执行流程如下
      2. 申请defer对象结构体，把结构体挂载到当前goruntime的G对象defer链上
      3. 保存defer调用的函数地址和参数到defer结构体中
      4. 返回到调用deferproc的地方，执行后面的语句
  
   - deferreturn
      1. deferreturn是在函数return处理中调用，执行流程如下：
      2. 在当前goruntime的G对象defer链上找需要处理的defer函数，没有就返回
      3. 把defer对象中的defer执行函数和参数拷贝到当前栈上
      4. 释放defer对象
      5. 通过jmpdefer命令执行defer函数

   - defer与return的执行顺序
     1. return开始，执行函数或者表达式
     2. 执行deferreturn
     3. 执行return，当前函数栈返回值整理
...
1. 主动panic
   ...
   - panic发生以后, 程序会执行runtime.sigpanic
   - 开始执行在panic函数前注册的defer函数，直到某个defer函数内的recover函数被执行，执行完所有defer函数以后，继续正常流程执行。 如果没有recover函数，执行完defer以后退出程序
   - panic嵌套, panic发生以后，开始执行defer，如果在defer又发生panic，就是panic嵌套，panic会产生对应panic结构，在当前g对象的panic对象链表，最近产生的在链表前，最开始的在链表尾部
...
2. 被动panic
   ...
   - 被动panic，在程序运行时，cpu执行发现错误，比如数组和切片访问越界。
   - cpu发生执行错误，陷入内核处理
   - 内核开始异常处理，保存发生错误的执行内存地址和寄存器值，给发生异常的线程发送SIGSEGV信号
   - 执行程序向内核注册异常处理函数
   - 执行完异常处理函数以后，内核异常处理返回
   - 返回用户态执行runtime.sigpanic函数，过程和主动panic一样
...
2. 内存分配
   ...
   - 小于16KB大小的对象在mcache上分配
   - 大于16KB小于32KB的对象现在mcache上找，如果没有合适的，在mcentral上找，如果没有合适的，再向mheap申请
   - 大于32KB的对象直接由mheap对象分配
