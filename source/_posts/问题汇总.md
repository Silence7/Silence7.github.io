...
1. 多个切片指向同一个切片引用时，修改谋个切片会导致所有切片被覆盖修改。解决方案：复制新切片通过copy函数
...
2. 切片的扩容，切片添加元素未触发扩容，会导致源切片值被覆盖修改。解决方案：新引用切片，需要指定长度=容量，再追加新内容时，触发扩容机制
